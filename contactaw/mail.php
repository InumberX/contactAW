...
$site_top = "http://after-works.2-d.jp/";

// 管理者メールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
$to = "inumberx@gmail.com";
...
$Referer_check_domain = "afterworks.jp";
...
$subject = "【AfterWorks】ホームページのお問い合わせ";
...
$thanksPage = "https://afterworks.jp/contactaw/thanks.html";
...
-------------------------------------------------------------
Name：numberix
Mail Address：inumberx@gmail.com
Home Page：https://afterworks.jp/
-------------------------------------------------------------
...
<title>After&ensp;Works.&ensp;|&ensp;お問い合わせ確認</title>
<!-- meta -->
<meta charset="utf-8" />
...
<div class="cnt-body">
<span class="sub-ttl">入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</span>
<div class="inq-form-tbl">

<?php echo $errm; ?>

<div class="form-btn-box submit-container">
<input type="button" value="戻る" class="btn-style btn-submit btn-back" onClick="history.back()">
</div>
...
<div class="cnt-body">
<span class="sub-ttl">以下の内容で間違いがなければ、「送信する」ボタンを押して下さい。</span>
...
<input type="submit" value="送信する" class="btn-style btn-submit">
<input type="button" value="戻る" onClick="history.back()" class="btn-style btn-back">
</div>
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\"><em>【".h($key)."】</em>は必須項目です。</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
		...
		if($existsFalg != 1){
				$res['errm'] .= "<p class=\"error_messe\"><em>【".$requireVal."】</em>が未選択です。</p>\n";
				$res['empty_flag'] = 1;
		}
	}
